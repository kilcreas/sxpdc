...
static struct topo_task topo_task[] = {
    ...
    TOPO_TASK_BINDING_CFG_DEL_INIT("add V4 binding to sxpd E", SXPD_E,
                                   "192.168.1.0", 24, PREFIX_IPV4, 42),
    TOPO_TASK_BINDING_CFG_DEL_INIT("add V6 binding to sxpd E", SXPD_E,
                                   "aaaa:eeee:abcf:eeee:0:0:0:0", 128,
                                   PREFIX_IPV6, 43),

    TOPO_TASK_WAIT_FOR_INIT(
        "wait for binding del re-exported from E to C", SXPD_C, WAIT_TIMEOUT,
        topo_task_cb_wait_for_bindings, &sxpd_bcd1_exp_bind_ctx),

    TOPO_TASK_WAIT_FOR_INIT(
        "wait for binding del re-exported from C to D", SXPD_D, WAIT_TIMEOUT,
        topo_task_cb_wait_for_bindings, &sxpd_bcd1_exp_bind_ctx),
    ...
    TOPO_TASK_WAIT_FOR_INIT(
        "wait for binding del re-exported from B to C", SXPD_C, WAIT_TIMEOUT,
