...
struct topo_task_peer_chk_ctx sxpd_b_exp_peer_ctx[] = {
    TOPO_TASK_WAIT_FOR_PEERS(NULL, 0, true),
...
static struct topo_task child1_task[] = {
    ...
    TOPO_TASK_WAIT_FOR_INIT("check B has no peers", SXPD_B, 0,
...
static struct topo_task child3_task[] = {
    ...
    TOPO_TASK_PEER_CFG_ADD_INIT("add peer both A to instance B", SXPD_B,
                                SXPD_A_IP, 64000, NULL, PEER_BOTH),
    TOPO_TASK_WAIT_FOR_INIT("wait for B A sxpd connect", SXPD_B, WAIT_TIMEOUT,
                            topo_task_cb_wait_for_peers,
...
static struct topo_task topo_task[] = {
    ...
    TOPO_TASK_MAIN_EXEC_INIT("fork process and run B tasks", topo_task_fork_cb,
                             &child1_fork_ctx),
    ...
    TOPO_TASK_WAIT_FOR_INIT("A B sxpd are not connected", SXPD_A, WAIT_TIMEOUT,
                            topo_task_cb_wait_for_peers,
                            &sxpd_a_exp_peer_ctx[0]),
    TOPO_TASK_MAIN_EXEC_INIT("again fork process and run B tasks",
                             topo_task_fork_cb, &child1_fork_ctx),
    TOPO_TASK_WAIT_FOR_INIT("wait for A B sxpd reconnect", SXPD_A, WAIT_TIMEOUT,
                            topo_task_cb_wait_for_peers,
                            &sxpd_a_exp_peer_ctx[1]),
    TOPO_TASK_MAIN_EXEC_INIT(
    ...
    TOPO_TASK_PEER_CFG_ADD_INIT("add peer both B to instance A", SXPD_A,
                                SXPD_B_IP, 64000, NULL, PEER_BOTH),
    TOPO_TASK_WAIT_FOR_INIT("A B sxpd are not connected", SXPD_A, WAIT_TIMEOUT,
                            topo_task_cb_wait_for_peers,
