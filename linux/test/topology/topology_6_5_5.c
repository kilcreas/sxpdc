...
#define RECONCILIATION_TIMER 10

/* test 6.5.3 - hold-down timer */
...
struct topo_task_peer_chk_ctx sxpd_c_exp_peer_ctx[] = {
    TOPO_TASK_WAIT_FOR_PEERS(
        sxpd_c_exp_peers,
        sizeof(sxpd_c_exp_peers) / sizeof(*sxpd_c_exp_peers) - 1, true),
    TOPO_TASK_WAIT_FOR_PEERS(sxpd_c_exp_peers, sizeof(sxpd_c_exp_peers) /
                                                   sizeof(*sxpd_c_exp_peers),
                             true),
};
...
struct topo_task_binding_chk sxpd_bcd1_exp_bindings[] = {
    TOPO_TASK_WAIT_FOR_BINDING("192.168.1.0", 24, PREFIX_IPV4, 10),
    TOPO_TASK_WAIT_FOR_BINDING("aaaa:eeee:abcf:eeeb:0:0:0:0", 64, PREFIX_IPV6,
                               10),
...
struct topo_task_binding_chk sxpd_cd2_exp_bindings[] = {
    TOPO_TASK_WAIT_FOR_BINDING("192.168.1.0", 24, PREFIX_IPV4, 14),
    TOPO_TASK_WAIT_FOR_BINDING("aaaa:eeee:abcf:eeeb:0:0:0:0", 64, PREFIX_IPV6,
                               14),
    TOPO_TASK_WAIT_FOR_BINDING("aaaa:eeee:abcf:eeee:0:0:0:0", 128, PREFIX_IPV6,
                               14),
...
/* expected bindings #3 */
struct topo_task_binding_chk sxpd_cd3_exp_bindings[] = {
    TOPO_TASK_WAIT_FOR_BINDING("192.168.1.0", 24, PREFIX_IPV4, 10),
    TOPO_TASK_WAIT_FOR_BINDING("aaaa:eeee:abcf:eeee:0:0:0:0", 128, PREFIX_IPV6,
                               14),
};

struct topo_task_binding_chk_ctx sxpd_cd3_exp_bind_ctx =
    TOPO_TASK_WAIT_FOR_BINDINGS(
        sxpd_cd3_exp_bindings,
        sizeof(sxpd_cd3_exp_bindings) / sizeof(*sxpd_cd3_exp_bindings), true);
...
static struct log_pattern log_pattern[] = {
    ...
    LOG_PATTERN_STATIC_INIT("expected trace about expired binding remove",
                            LOG_EXPECTED, LOG_LEVEL_DEBUG, "sxpd.c",
                            "sxpd_binding_is_expired",
                            "^(.*)(\\bDeleting expired\\b)(.*?)$"),
...
static struct log_pattern child_a_log_pattern[] = {
    LOG_PATTERN_STATIC_INIT("expected trace logs about uint32 config add",
                            LOG_EXPECTED, LOG_LEVEL_TRACE, "sxpd.c",
                            "sxpd_cfg_add_uint32_setting", NULL),
    LOG_PATTERN_STATIC_INIT("expected trace logs about peer add", LOG_EXPECTED,
...
static struct log_pattern child_e_log_pattern[] = {
    LOG_PATTERN_STATIC_INIT("expected trace logs about uint32 config add",
                            LOG_EXPECTED, LOG_LEVEL_TRACE, "sxpd.c",
                            "sxpd_cfg_add_uint32_setting", NULL),
    LOG_PATTERN_STATIC_INIT("expected trace logs about peer add", LOG_EXPECTED,
...
static struct topo_task_log_check_set_patterns_ctx child_a_log_patterns_ctx =
    TOPO_TASK_LOG_CHECK_SET_PATTERNS(child_a_log_pattern,
                                     sizeof(child_a_log_pattern) /
                                         sizeof(*child_a_log_pattern));

static struct topo_task_log_check_set_patterns_ctx child_e_log_patterns_ctx =
    TOPO_TASK_LOG_CHECK_SET_PATTERNS(child_e_log_pattern,
                                     sizeof(child_e_log_pattern) /
                                         sizeof(*child_e_log_pattern));

static struct topo_task child_a1_task[] = {
    TOPO_TASK_MAIN_EXEC_INIT("set log check patterns",
                             topo_task_log_check_set_patterns_cb,
                             &child_a_log_patterns_ctx),
    ...
    TOPO_TASK_UINT32_CFG_ADD_INIT("configure sxpd instance A enabled", SXPD_A,
                                  UINT32_SETTING_RECONCILIATION_TIMER,
...
static struct topo_task child_a2_task[] = {
    TOPO_TASK_MAIN_EXEC_INIT("set log check patterns",
                             topo_task_log_check_set_patterns_cb,
                             &child_a_log_patterns_ctx),
...
static struct topo_task child_e1_task[] = {
    TOPO_TASK_MAIN_EXEC_INIT("set log check patterns",
                             topo_task_log_check_set_patterns_cb,
                             &child_e_log_patterns_ctx),
    ...
    TOPO_TASK_BINDING_CFG_ADD_INIT("add V4 subnet binding to sxpd E", SXPD_E,
                                   "192.168.1.0", 24, PREFIX_IPV4, 14),
    TOPO_TASK_BINDING_CFG_ADD_INIT("add V6 host binding to sxpd E", SXPD_E,
                                   "aaaa:eeee:abcf:eeeb:0:0:0:0", 64,
                                   PREFIX_IPV6, 14),
    TOPO_TASK_BINDING_CFG_ADD_INIT("add V6 subnet binding to sxpd E", SXPD_E,
                                   "aaaa:eeee:abcf:eeee:0:0:0:0", 128,
                                   PREFIX_IPV6, 14),
...
static struct topo_task child_e2_task[] = {
    TOPO_TASK_MAIN_EXEC_INIT("set log check patterns",
                             topo_task_log_check_set_patterns_cb,
                             &child_e_log_patterns_ctx),
...
struct topo_task_fork_ctx child_a_fork_ctx[] = {
    TOPO_TASK_FORK_CTX_INIT(child_a1_task),
    TOPO_TASK_FORK_CTX_INIT(child_a2_task),
};

struct topo_task_fork_ctx child_e_fork_ctx[] = {
    TOPO_TASK_FORK_CTX_INIT(child_e1_task),
    TOPO_TASK_FORK_CTX_INIT(child_e2_task),
};

int sleep_before_reconciliation_timer_expire = RECONCILIATION_TIMER - 1;
...
static struct topo_task topo_task[] = {
    ...
    TOPO_TASK_MAIN_EXEC_INIT("fork process and run sxpd A tasks",
                             topo_task_fork_cb, &child_a_fork_ctx[0]),
    ...
    TOPO_TASK_PEER_CFG_ADD_INIT("add peer listener D to instance C", SXPD_C,
                                SXPD_D_IP, 64000, NULL, PEER_LISTENER),
    TOPO_TASK_PEER_CFG_ADD_INIT("add peer speaker C to instance D", SXPD_D,
                                SXPD_C_IP, 64000, NULL, PEER_SPEAKER),
    ...
    TOPO_TASK_MAIN_EXEC_INIT("fork process and run sxpd E tasks",
                             topo_task_fork_cb, &child_e_fork_ctx[0]),
    TOPO_TASK_PEER_CFG_ADD_INIT("add peer speaker E to instance C", SXPD_C,
                                SXPD_E_IP, 64000, NULL, PEER_SPEAKER),
    TOPO_TASK_WAIT_FOR_INIT("wait for C peers connect", SXPD_C, WAIT_TIMEOUT,
                            topo_task_cb_wait_for_peers,
                            &sxpd_c_exp_peer_ctx[1]),
    ...
    TOPO_TASK_MAIN_EXEC_INIT(
        "wait for A child process tasks and 'kill' process",
        topo_task_wait_child_cb, &child_a_fork_ctx[0]),
    TOPO_TASK_MAIN_EXEC_INIT(
        "wait for E child process tasks and 'kill' process",
        topo_task_wait_child_cb, &child_e_fork_ctx[0]),

    TOPO_TASK_MAIN_EXEC_INIT("fork process and restart sxpd A tasks",
                             topo_task_fork_cb, &child_a_fork_ctx[1]),
    TOPO_TASK_MAIN_EXEC_INIT("fork process and restart sxpd E tasks",
                             topo_task_fork_cb, &child_e_fork_ctx[1]),
    ...
    TOPO_TASK_MAIN_EXEC_INIT("sleep before reconciliation timer expire",
                             topo_task_sleep_cb,
                             &sleep_before_reconciliation_timer_expire),

    TOPO_TASK_WAIT_FOR_INIT("check if reconciliation timer is not expired yet",
                            SXPD_C, 0, topo_task_cb_wait_for_bindings,
                            &sxpd_cd2_exp_bind_ctx),

    TOPO_TASK_WAIT_FOR_INIT("wait for sxpd reconciliation timer expire", SXPD_C,
                            2, topo_task_cb_wait_for_bindings,
                            &sxpd_cd3_exp_bind_ctx),

    TOPO_TASK_WAIT_FOR_INIT("wait for binding del re-exported from C to D",
                            SXPD_D, 1, topo_task_cb_wait_for_bindings,
                            &sxpd_cd3_exp_bind_ctx),

    TOPO_TASK_MAIN_EXEC_INIT(
        "wait for A child process tasks and 'kill' process",
        topo_task_wait_child_cb, &child_a_fork_ctx[1]),
    TOPO_TASK_MAIN_EXEC_INIT(
        "wait for E child process tasks and 'kill' process",
        topo_task_wait_child_cb, &child_e_fork_ctx[1]),
