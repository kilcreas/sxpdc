...
/* c-java 4 - Linux vs Java SXP re-exporting bindings */
...
static const char topo_sxpd_ip[][16] = { "127.0.1.1", "127.0.1.2",
                                         "127.0.1.3" };
...
struct topo_task_peer_chk sxpd_b_exp_peers[] = {
    TOPO_TASK_WAIT_FOR_PEER(JAVA_SXPD_A_IP, PORT, true, true, false),
...
struct topo_task_peer_chk_ctx sxpd_c_exp_peer_ctx =
    TOPO_TASK_WAIT_FOR_PEERS(sxpd_c_exp_peers, 1, false);
...
static struct topo_task topo_task[] = {
    ...
    /* configure topology and test bindings re-export */
    ...
    TOPO_TASK_WAIT_FOR_INIT(
        "synchronization - bindings exported/re-exported/received", SXPD_A,
    ...
    TOPO_TASK_PEER_CFG_ADD_INIT("add peer speaker A to instance B", SXPD_B,
                                JAVA_SXPD_A_IP, PORT, PWD, PEER_SPEAKER),
