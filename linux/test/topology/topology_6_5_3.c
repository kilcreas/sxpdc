...
/* expected bindings #3 */
struct topo_task_binding_chk_ctx sxpd_bcd3_exp_bind_ctx =
...
static struct log_pattern child_b_log_pattern[] = {
    LOG_PATTERN_STATIC_INIT("expected trace logs about uint32 config add",
                            LOG_EXPECTED, LOG_LEVEL_TRACE, "sxpd.c",
                            "sxpd_cfg_add_uint32_setting", NULL),
    LOG_PATTERN_STATIC_INIT("expected trace logs about peer add", LOG_EXPECTED,
...
static struct topo_task_log_check_set_patterns_ctx child_b_log_patterns_ctx =
    TOPO_TASK_LOG_CHECK_SET_PATTERNS(child_b_log_pattern,
                                     sizeof(child_b_log_pattern) /
                                         sizeof(*child_b_log_pattern));

static struct topo_task child_a_task[] = {
...
static struct topo_task child_e_task[] = {
    TOPO_TASK_MAIN_EXEC_INIT("set log check patterns",
                             topo_task_log_check_set_patterns_cb,
                             &child_b_log_patterns_ctx),
...
struct topo_task_fork_ctx child_a_fork_ctx =
    TOPO_TASK_FORK_CTX_INIT(child_a_task);

struct topo_task_fork_ctx child_e_fork_ctx =
    TOPO_TASK_FORK_CTX_INIT(child_e_task);
...
static struct topo_task topo_task[] = {
    ...
    TOPO_TASK_MAIN_EXEC_INIT("fork process and run sxpd A tasks",
                             topo_task_fork_cb, &child_a_fork_ctx),
    ...
    TOPO_TASK_MAIN_EXEC_INIT("fork process and run sxpd E tasks",
                             topo_task_fork_cb, &child_e_fork_ctx),
    ...
    TOPO_TASK_MAIN_EXEC_INIT(
        ...
        topo_task_wait_child_cb, &child_e_fork_ctx),

    TOPO_TASK_WAIT_FOR_INIT(
        "wait for binding del re-exported from E to C", SXPD_C, HOLD_DOWN_TIMER,
        topo_task_cb_wait_for_bindings, &sxpd_bcd1_exp_bind_ctx),

    TOPO_TASK_WAIT_FOR_INIT(
        "wait for binding del re-exported from C to D", SXPD_D, HOLD_DOWN_TIMER,
        topo_task_cb_wait_for_bindings, &sxpd_bcd1_exp_bind_ctx),

    TOPO_TASK_MAIN_EXEC_INIT(
        "wait for A child process tasks and 'kill' process",
        topo_task_wait_child_cb, &child_a_fork_ctx),

    TOPO_TASK_WAIT_FOR_INIT("wait for binding del exported from A to B", SXPD_B,
                            HOLD_DOWN_TIMER, topo_task_cb_wait_for_bindings,
                            &sxpd_bcd3_exp_bind_ctx),

    TOPO_TASK_WAIT_FOR_INIT(
        "wait for binding del re-exported from B to C", SXPD_C, HOLD_DOWN_TIMER,
        topo_task_cb_wait_for_bindings, &sxpd_bcd3_exp_bind_ctx),
