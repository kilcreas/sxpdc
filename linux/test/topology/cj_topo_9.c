...
/* c-java 9 - Linux vs Java SXP mode mismatch testing */
...
static struct topo_task_peer_chk sxpd_a_exp_peers[] = {
    ...
    TOPO_TASK_WAIT_FOR_PEER(SXPD_B_IP, PORT, false, false, true),
};
...
static struct log_pattern log_pattern[] = {
    ...
    LOG_PATTERN_STATIC_INIT("expected error logs", LOG_OPTIONAL,
...
static struct topo_task topo_task[] = {
    ...
    /* configure topology and test mode mismatch */
    ...
    TOPO_TASK_WAIT_FOR_INIT(
        "synchronization - A(speaker) and B(speaker) mode mismatch", SXPD_A,
        WAIT_TIMEOUT, topo_task_wait_for_sync_cb, &sync_ctx),
