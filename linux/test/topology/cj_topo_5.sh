...
conflictManagementTest(){
	...
	waitUntil 10 containsConnection "127.0.2.4" "127.0.1.3" 64999 "on"; then
		echo -e "$(tput setaf 1)\tTEST FAILURE$(tput sgr0)";
		cleanUp;
		return;
	fi
   
	if waitUntil 10 containsBinding "127.0.2.4" 40 "192.168.1.1/32" ||
	waitUntil 10 containsBinding "127.0.2.4" 42 "192.168.2.0/24" ||
	waitUntil 10 containsBinding "127.0.2.4" 60 "aaaa:eeee:abcf:eeee:0:0:0:0/128" ||
	waitUntil 10 containsBinding "127.0.2.4" 62 "aaaa:eeee:abcf:eeed:0:0:0:0/64"; then
		echo -e "$(tput setaf 1)\tTEST FAILURE$(tput sgr0)";
		cleanUp;
		return;
	fi
	...
	if waitUntil 10 containsConnection "127.0.2.5" "127.0.1.3" 64999 "on"; then
		echo -e "$(tput setaf 1)\tTEST FAILURE$(tput sgr0)";
		cleanUp;
		return;
	fi
	...
	synchronize 60 $SYNC "Bindings conflict management 1";		
	echo -e "$(tput setaf 2)\tTEST PASS$(tput sgr0)";
   	cleanUp;
};

conflictManagementTest2(){
	synchronize 60 $SYNC "Init";		
	...
	synchronize 60 $SYNC "Topology setup";

	addConnection "127.0.1.5" 64999 "listener" 4 "none" 127.0.2.3
	if waitUntil 10 containsConnection "127.0.2.3" "127.0.1.5" 64999 "on"; then
		echo -e "$(tput setaf 1)\tTEST FAILURE$(tput sgr0)";
		cleanUp;
		return;
	fi

	synchronize 60 $SYNC "Last peer added";
	synchronize 60 $SYNC "Bindings conflict management 2";
	echo -e "$(tput setaf 2)\tTEST PASS$(tput sgr0)";
	cleanUp;
};

echo -e "Verify that shortest peer-sequence rule is used\n"

cleanUp;
./cj_topo_5 $SYNC 2> sxp5.log &
conflictManagementTest;
conflictManagementTest2;
