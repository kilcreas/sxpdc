...
#define EXPANSION_LIMIT 4
/* test 6.8.3 - expansion limit test */
...
struct topo_task_binding_chk sxpd_a_exp_bindings[] = {
    ...
    TOPO_TASK_WAIT_FOR_BINDING("192.168.2.0", 29, PREFIX_IPV4, 29),
};

struct topo_task_binding_chk_ctx sxpd_a_exp_bind_ctx[] = {
    TOPO_TASK_WAIT_FOR_BINDINGS(sxpd_a_exp_bindings, 1, true),
...
struct topo_task_binding_chk sxpd_b_exp_bindings[] = {
    TOPO_TASK_WAIT_FOR_BINDING("192.168.1.1", 32, PREFIX_IPV4, 30),
...
static struct log_pattern log_pattern[] = {
    ...
    LOG_PATTERN_STATIC_INIT("expected alert about expansion limit reached",
                            LOG_EXPECTED, LOG_LEVEL_ALERT, "sxpd.c",
                            "sxpd_expand_binding",
                            "^(.*)(\\bWarning: could not expand all bindings, "
                            "current limit for expansion is 4 expanded host "
                            "entries\\b)(.*?)$"),
...
static struct topo_task topo_task[] = {
    ...
    TOPO_TASK_UINT32_CFG_ADD_INIT("configure sxpd instance A node id", SXPD_A,
                                  UINT32_SETTING_SUBNET_EXPANSION_LIMIT,
                                  EXPANSION_LIMIT),
    ...
    TOPO_TASK_BINDING_CFG_ADD_INIT("add ipv4 subnet binding to sxpd A", SXPD_A,
                                   "192.168.1.0", 30, PREFIX_IPV4, 30),

    TOPO_TASK_WAIT_FOR_INIT("check binding list", SXPD_A, 0,
                            topo_task_cb_wait_for_bindings,
                            &sxpd_a_exp_bind_ctx[0]),
    ...
    TOPO_TASK_BINDING_CFG_ADD_INIT("add ipv4 subnet binding which has more "
                                   "host entries than the configured expansion "
                                   "limit to sxpd A",
                                   SXPD_A, "192.168.2.0", 29, PREFIX_IPV4, 29),

    TOPO_TASK_WAIT_FOR_INIT("check binding list", SXPD_A, 0,
                            topo_task_cb_wait_for_bindings,
                            &sxpd_a_exp_bind_ctx[1]),

    TOPO_TASK_MAIN_EXEC_INIT("sleep", topo_task_sleep_cb, &sleep_time),

    TOPO_TASK_WAIT_FOR_INIT(
        "check if no new bindings has been exported from A to B", SXPD_B, 0,
        topo_task_cb_wait_for_bindings, &sxpd_b_exp_bind_ctx),

    TOPO_TASK_BINDING_CFG_DEL_INIT("del V4 binding from sxpd A", SXPD_A,
                                   "192.168.1.0", 30, PREFIX_IPV4, 30),
    TOPO_TASK_BINDING_CFG_DEL_INIT("del V4 binding from sxpd A", SXPD_A,
                                   "192.168.2.0", 29, PREFIX_IPV4, 29),
